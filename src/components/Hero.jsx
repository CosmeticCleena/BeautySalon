...
const Hero = () => {
  ...
  const handleTouchStart = (e) => {
    if (heroImage.length > 1) setTouchStart(e.targetTouches[0].clientX);
  };

  const handleTouchMove = (e) => {
    if (heroImage.length > 1) setTouchEnd(e.targetTouches[0].clientX);
  };

  const handleTouchEnd = () => {
    if (heroImage.length <= 1) return;
  ...
  return (
    <div
      ref={heroRef}
      className="w-screen aspect-[16/9] bg-cover bg-center -z-10 overflow-hidden mt-[82px]"
    ...
    >
      <div className="flex flex-col items-center justify-center h-full w-screen px-4 md:px-0">
        ...
        {/* Điểm đánh dấu vị trí ảnh - chỉ hiển thị nếu có nhiều hơn 1 ảnh */}
        {heroImage.length > 1 && (
        ...
        {/* Hướng dẫn swipe - chỉ hiển thị trên mobile nếu có nhiều hơn 1 ảnh */}
        {heroImage.length > 1 && (
