...
const Hero = () => {
  ...
  before:bg-white before:bg-opacity-50 before:content-[''] before:-z-9 before:top-0 before:left-0"
        ...
        <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between px-24 z-20">
          <button onClick={() => setHeroImageIndex((heroImageIndex - 1 + heroImage.length) % heroImage.length)}>
            <img src={leftButton} alt="" className="h-16 w-16 p-5 rounded-full bg-[#0D0D0D80] opacity-50 hover:opacity-90"/>
          </button>
          <button onClick={() => setHeroImageIndex((heroImageIndex + 1) % heroImage.length)}>
            <img src={rightButton} alt="" className="h-16 w-16 p-5 rounded-full bg-[#0D0D0D80] opacity-50 hover:opacity-90"/>
          </button>
        </div>
        <div className="absolute bottom-[40px] -translate-y-1/2 flex justify-center gap-8">
            {heroImage.map((image, index) => (
              (<div className={`w-12 border-4 border-white rounded-full ${heroImageIndex == index ? 'border-[#D1AE62]' : ''}`} key={index}>
